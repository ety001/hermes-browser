...
> 修订记录：v4 修正 glm5turbo-v3 审计 P0#3(execute_js 参数名 code→expression)、P2#7(错误码补全)；
> v3 修正 P0#1(DefaultBoolean→DefaultBool)、P0#4(参数访问用 GetString)；
> v2 修正 P2#10(错误码)、P2#11(full_page TODO)
...
- `TAB_CLOSED` - Tab 已关闭，建议使用其他 tab
- `NO_EXTENSION_CONNECTED` - Extension 未连接，建议提示用户检查
- `PERMISSION_DENIED` - 权限不足，检查 tab 是否激活
- `UNKNOWN_METHOD` - 未知命令
...
mcp.NewTool("execute_js",
    ...
    mcp.WithString("expression",  // P0#3: 统一参数名为 expression
