# Chrome Extension 详细设计（v4）

> 修订记录：v4 修正 glm5turbo-v3 审计 P1#4(navigate timeout 传递到 waitForTabComplete)；
> v3 修正 P1#8(execute_js 异步支持)、P1#9(tab_id 参数传递)、P2#12(waitForTabComplete 超时标记)
...
async function handleNavigate(tabId, params) {
    ...
    // 等待页面加载完成
    const timeoutMs = timeout || 60000;  // P1#4: 使用 MCP 传入的 timeout
