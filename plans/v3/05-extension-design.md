# Chrome Extension 详细设计（v3）

> 修订记录：v3 修正 glm5turbo-v2 审计 P1#8(execute_js 异步支持)、P1#9(tab_id 参数传递)、
> P2#12(waitForTabComplete 超时标记)；
> v2 修正 P0#2(navigate 归 Background)、P1#4(screenshot 归 Background)、P1#6(cleanText)、
> P1#7(networkidle)、P1#8(activeTab)、P2#13(删除 content_scripts)、P2#14(icons)、P2#17(命令路由表)
...
function waitForTabComplete(tabId, timeoutMs = 60000) {
    return new Promise((resolve, reject) => {
        ...
        // P2#12: 超时保护，超时返回 error 而非静默 resolve
        setTimeout(() => {
            chrome.tabs.onUpdated.removeListener(listener);
            resolve(); // 超时也返回，不阻塞（但调用方应检查 tab.status）
        }, timeoutMs);
    });
}
```
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (method) {
        ...
        case 'execute_js':
            ...
            return true; // P1#8: 异步响应，支持 await/async 用户代码
...
### JavaScript 执行（P1#8: 异步支持）

```javascript
// executeJavaScript 必须是 async 函数，以支持用户代码中的 await
async function executeJavaScript(params) {
    const { expression, return_value = true } = params;

    try {
        // 使用 async IIFE 包裹用户代码，支持顶级 await
        const wrappedCode = return_value
            ? `(async () => { return await (async () => { ${expression} })(); })()`
            : `(async () => { ${expression} })()`;
...
**注意：**
- 使用 `async IIFE` 包裹用户代码，使得用户可以写 `const data = await fetch(...)` 这样的代码
- `eval` 在 content script 中可用（注意 CSP 限制，部分页面如 chrome:// 无法注入）
- 如果页面 CSP 严格禁止 eval，需要降级到 `new Function` 方案
