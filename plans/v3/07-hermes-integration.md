...
> 修订记录：v3 修正 glm5turbo-v2 审计 P0#3(Hermes config.yaml 格式，headers 字段传递认证)；
> v2 修正 P0#1 URL 路径与自定义 mux 一致

## Hermes config.yaml 配置

在 Hermes 的 `~/.hermes/config.yaml` 中添加 MCP server 配置。
Hermes 通过 `url` 字段的存在性判断 transport 类型（有 url = HTTP，无 url = stdio）。
不需要 `transport` 字段。认证通过 `headers` 字典传递，支持 `${ENV_VAR}` 环境变量插值。
...
1. 设置环境变量（供 Hermes 使用）：
   ```bash
   export MCP_BROWSER_TOKEN="your-chosen-token"
   ```
2. 在 MCP Server 的 `config.yaml` 中设置相同 token：
   ```yaml
   token: "your-chosen-token"
   ```
3. 或者不配置 token，首次启动时自动生成并保存到 `~/.hermes-browser/.token`，
   从该文件读取值设置到环境变量

### 自动生成 Token

如果 `token` 配置为空，MCP Server 首次启动时自动生成 64 字符 hex token，
保存到 `~/.hermes-browser/.token` 文件并打印到 stderr。
