# Go MCP Server 详细设计（v3）

> 修订记录：v3 修正 glm5turbo-v2 审计 P0#2(去掉 WithToolHandler)、P0#4(参数访问用 GetString)、
> P0#5(ws 引用 auth)、P1#6(超时从 config 读取)、P1#7(Hub 共享、main.go 桥接)、P2#13(确认 NewToolResultImage 签名)；
> v2 修正 P0#1(中间件包装)、P0#3(Hub 单 client)、P2#9(coder/websocket)、P2#10(错误码)、P2#15(stdin 检测)
