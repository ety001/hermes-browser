# Go MCP Server 详细设计（v2）

> 修订记录：根据审计意见修正 P0#1(中间件包装)、P0#3(Hub 单 client)、P2#9(coder/websocket)、P2#10(错误码)、P2#15(stdin 检测)
...
`)
    ...
    if cfg.Server.HTTP.Bind != "" {
        // HTTP transport 模式（v2：自定义 mux，不用 httpServer.Start）
        startHTTPServer(ctx, cfg, mcpServer)
    } else {
        // stdio transport 模式
        startStdioServer(ctx, mcpServer)
    }
...
```go
func startHTTPServer(ctx context.Context, cfg *config.Config, mcpServer *mcp.Server) {
    // 创建 StreamableHTTPServer，但不调用 Start()
...
func authMiddleware(next http.Handler, cfg *config.Config) http.Handler {
    return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        ...
        expected := cfg.GetHTTPToken()

        if !auth.Validate(provided, expected) {
...
- 生命周期管理：自己建 `http.Server`，调用 `sseServer.Shutdown(ctx)` 关闭 MCP 部分
...
func NewServer(cfg *config.Config) *Server {
    ...
    s.mcpS = server.NewMCPServer(
        "hermes-browser",
        "1.0.0",
        server.WithToolHandler(s.handleToolCall),
    )
...
type Hub struct {
    mu        sync.RWMutex
    client    *Client        // 单一 Extension 客户端（v2 简化）
...
func StartServer(ctx context.Context, cfg *config.Config, hub *Hub) *http.Server {
    ...
    mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
        ...
        if !isAllowedOrigin(origin, cfg.WebSocket.AllowedExtensions) {
...
func (s *Server) handleNavigate(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
    ...
    waitUntil, _ := req.Params.Arguments["wait_until"].(string)
    if waitUntil == "" {
        waitUntil = s.cfg.Browser.DefaultWaitUntil
    }
    timeout, _ := req.Params.Arguments["timeout"].(float64)

    // 获取当前激活 tab（如果未指定 tab_id）
    ...
    if err != nil {
        if err == ws.ErrNoExtensionConnected {
            return mcp.NewToolResultText("Error: Chrome Extension is not connected. Please check if the Hermes Browser extension is running."), nil
        }
        return mcp.NewToolResultText(fmt.Sprintf("Error: %v", err)), nil
    }

    if resp.Status == "error" {
        return mcp.NewToolResultText(fmt.Sprintf("Error [%s]: %s", resp.Code, resp.Error)), nil
    }

    data, _ := resp.Data.(map[string]interface{})
    return mcp.NewToolResultText(fmt.Sprintf("Navigated to %s\nTitle: %s", data["url"], data["title"])), nil
}
...
func (s *Server) handleScreenshot(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
    ...
    quality, _ := req.Params.Arguments["quality"].(float64)
    ...
    // 返回 MCP ImageContent
    return mcp.NewToolResultImage(imageBase64, "image/jpeg"), nil
}
```
...
require (
    ...
    github.com/coder/websocket v1.8.12    // v2: 替代 gorilla/websocket
