...
// startStdioServer 使用 stdio transport 启动 MCP Server
func startStdioServer(mcpServer *mcp.Server) {
    sseServer := server.NewStreamableHTTPServer(
        mcpServer.MCPServer(),
    )
    // stdio transport：将 MCP 请求从 stdin 转发给 MCPServer
    // mcp-go 的 stdio 模式需要通过 NewStdioServer 或手动桥接
    fmt.Fprintf(os.Stderr, "MCP server running in stdio mode\n")
    // TODO: 使用 mcp-go 提供的 stdio transport adapter
    // 参考方案：直接将 os.Stdin/os.Stdout 传给 sseServer 或使用 server.NewStdioServer
}
```
...
// 15 个 MCP 工具定义变量（P1#7: 对应 03-mcp-tools.md 中的 NewTool 调用）
var (
    navigateTool = mcplib.NewTool("navigate",
    ...
    screenshotTool = mcplib.NewTool("screenshot",
    ...
    getContentTool = mcplib.NewTool("get_content",
    ...
    clickTool = mcplib.NewTool("click",
    ...
    executeJsTool = mcplib.NewTool("execute_js",
    ...
    waitForTool = mcplib.NewTool("wait_for",
    ...
    listTabsTool = mcplib.NewTool("list_tabs",
        mcplib.WithDescription("List all open browser tabs"),
    )

    switchTabTool = mcplib.NewTool("switch_tab",
    ...
    closeTabTool = mcplib.NewTool("close_tab",
    ...
    newTabTool = mcplib.NewTool("new_tab",
    ...
    hoverTool = mcplib.NewTool("hover",
    ...
    selectOptionTool = mcplib.NewTool("select_option",
...
func (s *Server) handleScreenshot(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
    ...
    if quality, ok := args["quality"].(float64); ok && quality > 0 {
        ...
        params["quality"] = s.cfg.Browser.ScreenshotQuality  // P2#5: 使用配置默认值
    }
    if selector != "" {
...
type HTTPConfig struct {
    ...
    Token string `yaml:"token"` // 可选，覆盖顶层 token
}
...
type BrowserConfig struct {
    ...
    MaxContentLength  int    `yaml:"max_content_length"` // chars, default 500000
}
...
func Load() (*Config, error) {
    ...
    cfg := &Config{
        Browser: BrowserConfig{
            ...
            MaxContentLength:  500000,  // P0#4: 与 02-config-design.md 一致
        },
...
// 优先级：server.http.token > 顶层 token > 环境变量 HB_TOKEN
func (c *Config) GetHTTPToken() string {
    ...
    return os.Getenv("HB_TOKEN")
}

// GetWebSocketToken 返回 WebSocket 认证 token
// 优先级：websocket.token > 顶层 token > 环境变量 HB_TOKEN
